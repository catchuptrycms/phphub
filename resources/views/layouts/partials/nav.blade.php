    ...
    <div class="container">
        <div class="navbar-header hidden-xs">
            ...
            <a href="/" class="navbar-brand">
                <img src="{{ cdn('assets/images/logo4.png') }}" alt="Laravel"/>
            </a>
        </div>
        <div id="top-navbar-collapse" class="navbar-collapse">
            <ul class="nav navbar-nav">
                ...
                <li class="{{ Request::is('categories/1') ? ' active' : '' }}"><a
                            href="{{ route('categories.show', 1) }}">{{ lang('Jobs') }}</a></li>
                <li class="{{ (Request::is('wiki') ? ' active' : '') }}"><a href="{{ route('wiki') }}">Wiki</a></li>

                @if(Auth::check() && Auth::user()->can('access_board'))
                    <li class="{{ Request::is('categories/'.config('app.admin_board_cid')) ? ' active' : '' }}"><a
                                href="{{ route('categories.show', config('app.admin_board_cid')) }}">Board CID</a></li>
                @endif
            </ul>
            ...
            <div class="navbar-right">
                <form method="GET" action="{{route('search')}}" accept-charset="UTF-8" class="navbar-form navbar-left"
                    ...
                    <div class="form-group">
                        <input class="form-control search-input mac-style" placeholder="{{lang('Search')}}" name="q"
                               type="text">
                    </div>
                </form>
                ...
                <ul class="nav navbar-nav github-login">
                    @if (Auth::check())
                        <li>
                            <a href="{{ isset($category) ? URL::route('topics.create', ['category_id' => $category->id]) : URL::route('topics.create') }}"
                               data-placement="bottom" class="popover-with-html" data-content="Add a Topic">
                                <i class="fa fa-plus text-md"></i>
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('notifications.index') }}" class="text-warning" style="margin-top: -4px;">
                                <span class="badge badge-{{ $currentUser->notification_count > 0 ? 'important' : 'fade' }} popover-with-html"
                                      data-content="message notification" id="notification-count">
                                ...
                                @if (Auth::user()->can('visit_admin'))
                                    <li>
                                        <a href="/admin" class="no-pjax">
                                            <i class="fa fa-tachometer text-md"></i> Admin
                                        </a>
                                    </li>
                                @endif
                                ...
                                <li>
                                    <a class="button" href="{{ route('users.show', $currentUser->id) }}"
                                       data-lang-loginout="{{ lang('Are you sure want to logout?') }}">
                                        <i class="fa fa-user text-md"></i> Show User
                                    </a>
                                </li>
                                <li>
                    ...
                    @else
                        <a href="{{ URL::route('auth.oauth', ['driver' => 'wechat']) }}"
